...
$select_ad_table = mysql_query("select * from house where house_id = '$house_id'");
 ...
 <div class="col-lg-10 col-md-9 col-sm-9 col-xs-12"> 
    <div class='row'> 
		...
		<div class='col-lg-2 col-md-2 col-sm-2 col-xs-12'>	
			<h4>&nbsp;</h4>
                <a href="house.php"><h5 style="background:#1e8032; padding:9px; width:80%; color:#fff; text-align:center; border:solid 1px #CCC; border-radius:5px; margin-left:2%;"><i class=""></i>Add</h5></a>
		</div>
		<div class='col-lg-2 col-md-2 col-sm-2 col-xs-12'>		
				 <h4>&nbsp;</h4>
                <a href="house_update_req.php"><h5 style="background:#fc7d0d; padding:9px; width:80%; color:#fff; text-align:center; border:solid 1px #CCC; border-radius:5px; margin-left:2%;"><i class=""></i>Edit</h5></a>
              
			
		</div>
		<div class='col-lg-2 col-md-2 col-sm-2 col-xs-12'>		
				 <h4>&nbsp;</h4>
                <a href="house_delete.php"><h5 style="background:#bb2107; padding:9px; width:80%; color:#fff; text-align:center; border:solid 1px #CCC; border-radius:5px; margin-left:2%;"><i class=""></i>Delete</h5></a>
    ...
    <div class="row">
      ...
      <div class="panel-heading" style='background:#000000; color:#fff;'>Update house</div>
	...
	<form action="" method ="POST" enctype="multipart/form-data">
	    ...
	    <div class="form-group">
	        ...
	        <input type="text" class="form-control" id="title" placeholder="house tilte" name="title" value="<?php echo  $cat_row['house_title']; ?>"></input>
	    </div>
			    <div class="form-group">
	        <label for="size">Size</label>
	        <input type="text" class="form-control" id="size" placeholder="size" name="size" value="<?php echo  $cat_row['house_size']; ?>">
	    </div>
			    <div class="form-group">
	        <label for="house_size">house size in</label><br>
	       <select name="house_size"  class="form-control">
		   <option value='<?php echo  $cat_row['house_perches']; ?>'><?php echo  $cat_row['house_perches']; ?></option>
    ...
    <option value="acres">Acres</option>
	        ...
	        <textarea type="text" class="form-control" id="description" placeholder="description" name="description" value=""><?php echo  $cat_row['house_desc']; ?></textarea>
	    </div>
			    <div class="form-group">
	        <label for="price">Price</label>
	        <input type="text" class="form-control" id="price" placeholder="price" name="price" value="<?php echo  $cat_row['house_price']; ?>">
			<p style='color:red;'>*Don't Make the Space between the price</p>
	    </div>
			    <div class="form-group">
	        <label for="address">Address</label>
	        <input type="text" class="form-control" id="address" placeholder="address" name="address" value="<?php echo  $cat_row['house_address']; ?>">
	    </div>
			    <div class="form-group">
	        <label for="location">Location</label> 
			
	        <select class="form-control" type="text"  name="location" >
				<option value='<?php echo  $cat_row['house_location']; ?>'><?php echo  $cat_row['house_location']; ?></option>
		 ...
		 <div class="form-group">
	       ...
	       <input type="text" class="form-control" id="city" placeholder="city" name="city" value='<?php echo  $cat_row['house_city']; ?>'>
		...
		<div class="col-lg-6">
		 <div class="form-group">
	        ...
	        <input type="tel" class="form-control" id="number"  pattern="^\d{3}\d{4}\d{3}$" placeholder="contact number" name="number" value="<?php echo  $cat_row['house_contact']; ?>">
		 ...
		 <div class="form-group">
	        <label for="state">house state</label>
			 <select name="state"  class="form-control">
				<option value='<?php echo  $cat_row['house_state']; ?>'><?php echo  $cat_row['house_state']; ?></option>
	    ...
	    <div class="form-group">
	        ...
	        <select name="electri"  class="form-control">
	        	<option value='<?php echo  $cat_row['electricity']; ?>'><?php echo  $cat_row['electricity']; ?></option>
				<option value="Available">Available</option>
				<option value="Not Available">Not Available</option>		   
			</select>
	    </div>
		 <div class="form-group">
	        ...
	        <select name="water"  class="form-control">
	        	<option value='<?php echo  $cat_row['tap_water']; ?>'><?php echo  $cat_row['tap_water']; ?></option>
				<option value="Available">Available</option>
				<option value="Not Available">Not Available</option>		   
			</select>
	    </div>
		 <div class="form-group">
	        ...
	        <select name="road"  class="form-control">
	        	<option value='<?php echo  $cat_row['wide_road']; ?>'><?php echo  $cat_row['wide_road']; ?></option>
	    ...
	    <a href="house_update_req.php" class="btn btn-default" role="button" >Back</a>
		...
		if(isset($_POST['submit']))
		{
			...
			{*/
				...
				/*$query = mysqli_query($con,"insert into house(house_title,house_size,house_perches,house_desc,house_price,house_address,house_location,house_city,house_contact,house_state,electricity,tap_water,wide_road,image_name,image_path,image_type,image_name1,image_path1,image_type1,image_name2,image_path2,image_type2,image_name3,image_path3,image_type3)values('$title','$size','$house_size','$description','$price','$address','$location','$city','$number','$state','$electri','$water','$road','$filename','$filepath','$filetype','$filename1','$filepath1','$filetype1','$filename2','$filepath2','$filetype2','$filename3','$filepath3','$filetype3')");
				if($query)
				{
			...
			$sql = "UPDATE house SET house_title='$title',house_size='$size',house_perches='$house_size',house_desc='$description',house_price='$price',house_address='$address',house_location='$location',house_city='$city',house_contact='$number',house_state='$state',electricity='$electri',tap_water='$water',wide_road='$road',image_name='$filename',image_path='$filepath',image_type='$filetype',image_name1='$filename1',image_path1='$filepath1',image_type1='$filetype1',image_name2='$filename2',image_path2='$filepath2',image_type2='$filetype2',image_name3='$filename3',image_path3='$filepath3',image_type3='$filetype3' WHERE house_id = '$house_id'";
